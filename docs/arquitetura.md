...
- Planos, correções de redação e provas gerados pelo LLM podem gravar uma trilha best-effort em `audit_log` com `action='teacher_artifact.create'`, `target_type='teacher_artifact'` e `metadata` contendo `actorUserId`, `kind`, `title`, parâmetros, conteúdo limitado e metadados do modelo. Os writes ficam atrás de `TEACHER_ARTIFACTS_AUDIT_LOG=1` para não bloquear geração em produção enquanto não existir tabela dedicada.
