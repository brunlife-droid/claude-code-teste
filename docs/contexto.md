...
- Etapa 1 do UX do chat do aluno aplicada: header pedagógico com status, ações rápidas, composer multilinha, bolhas da tutora mais ricas, fontes RAG mais legíveis, layout mobile com navegação inferior e seletor demo oculto em telas pequenas.
...
- **Chat multimodal do aluno**: `/api/chat` aceita anexos reais (`image`, `audio`, `document`) vindos do upload. Imagens são carregadas do storage S3 privado e enviadas ao gateway como parte multimodal; áudios são transcritos via OpenAI quando `OPENAI_API_KEY` está presente; PDFs/DOCX/TXT/MD têm texto extraído antes da chamada LLM. A mensagem do aluno persiste metadata do anexo em `messages.attachments`, e a ação é auditada em `audit_log` como `student.chat.attachment_analyze`. A UI agora traz status pedagógico, ações rápidas, composer multilinha e experiência mobile dedicada.
